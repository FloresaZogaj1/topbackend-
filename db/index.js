...
if (isTrue(process.env.DB_SSL)) {
  ...
  cfg.ssl = caPath
    ...
    : { minVersion: "TLSv1.2", rejectUnauthorized: false };          // ⚠️ fallback dev
}

const pool = mysql.createPool(cfg);

// Self-test log për të parë ku po lidhet serveri
(async () => {
  try {
    const [[info]] = await pool.query(
      "SELECT DATABASE() AS db, @@hostname AS host, NOW() AS now"
    );
